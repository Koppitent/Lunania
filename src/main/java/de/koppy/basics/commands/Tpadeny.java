        ...
        if(args.length == 1) {
            ...
            if(Bukkit.getPlayer(name) != null) {
                ...
                if(Tpa.existInRequest(p, targettodadd)) {
                    ...
                    targettodadd.sendMessage(LunaniaSystem.getServerInstance().getPrefix() + tprofile.getMessage("teleportdeniedfrom").replace("%player%", p.getName()));
                    p.sendMessage(LunaniaSystem.getServerInstance().getPrefix() + profile.getMessage("tpdeniedfrom").replace("%player%", targettodadd.getName()));
                }else {
