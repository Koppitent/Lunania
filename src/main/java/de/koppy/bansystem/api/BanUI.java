...
public class BanUI extends UI {

    private UUID uuid;
    private String bannedbyuuid;

    public BanUI(UUID uuid, String bannedbyuuid) {
        super("§e" + Bukkit.getOfflinePlayer(uuid).getName() + "'s §cBanMenu", 9*4);
        this.uuid = uuid;
    ...
    public void setMainMenu() {
        ...
        inventory.setItem(20, new ItemBuilder(Material.ANVIL).setDisplayname("§bBan by ID").getItemStack());
        inventory.setItem(24, new ItemBuilder(Material.CLOCK).setDisplayname("§bBan by time").getItemStack());
    }

    public void setIdList() {
        inventory.clear();
        setGlassRand(new ItemBuilder(Material.BLACK_STAINED_GLASS_PANE).setDisplayname("§c").getItemStack());
        for(BanIDs banids : BanIDs.banids.values()) {
            inventory.addItem(new ItemBuilder(banids.getMaterial()).setDisplayname("§c#"+banids.getId()).setLocalizedName("#"+banids.getId()).addLore("§c" + banids.getReason()).addLore("§7").addLore("§3"+banids.getTimeDuration()).getItemStack());
        }
